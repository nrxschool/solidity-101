...
describe("Token", function () {
  ...
  describe("Minting", function () {
    ...
    it("Should make sure that minted tokens are equal to total supply after each mint", async function () {
      ...
      expect(await token.balanceOf(other.address)).to.be.equal(mintAmount);
    });
  });

  describe("Roles", function () {
    it("Should allow only ADMIN to grant MINTER & PAUSER role", async function () {
      const { token, admin, other, owner } = await loadFixture(deployTokenFixture);

      await expect(token.connect(other).grantAdmin(admin.address)).to.be.reverted
      await expect(token.connect(owner).grantAdmin(other.address)).not.to.be.reverted
    });
    it("Should allow only ADMIN to grant MINTER & PAUSER role", async function () {
  ...
  describe("Burn", function () {
    it("Anyone should be able to burn tokens", async function () {
      ...
      expect(await token.balanceOf(other.address)).to.be.equal(parseEther("500"));
    });
